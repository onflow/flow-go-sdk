...
func demo() {
	...
	blockHeadersChan, errChan, initErr := flowClient.SubscribeBlockHeadersFromStartBlockID(ctx, header.ID, flow.BlockStatusFinalized)
	...
	for {
		...
		case blockHeader, ok := <-blockHeadersChan:
			if !ok {
				panic("block header channel is closed")
			}
			examples.Print(blockHeader)
		case err, ok := <-errChan:
			if !ok {
