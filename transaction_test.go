...
func TestTransaction_RLPMessages(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			tx: func(baseTx *flow.Transaction) {
				baseTx.SetReferenceBlockID(flow.EmptyID)
			},
		...
		},
		{
			...
			tx: func(baseTx *flow.Transaction) {
				baseTx.SetProposalKey(flow.HexToAddress("01"), 0, 10)
			},
		...
		},
		{
			...
			tx: func(baseTx *flow.Transaction) {
				baseTx.SetProposalKey(flow.HexToAddress("01"), 4, 0)
			},
		...
		},
		{
			...
			tx: func(baseTx *flow.Transaction) {
				baseTx.AddAuthorizer(flow.HexToAddress("02"))
			},
		...
		},
		{
			...
			tx: func(baseTx *flow.Transaction) {
				baseTx.AddRawArgument(jsoncdc.MustEncode(cadence.NewString("foo")))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			payload := hex.EncodeToString(tx.PayloadMessage())
			envelope := hex.EncodeToString(tx.EnvelopeMessage())
